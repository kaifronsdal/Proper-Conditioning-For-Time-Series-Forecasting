...
class FiLM(nn.Module):
    ...
    def __init__(
        ...
        self.gamma_layer = nn.Linear(input_dimension, output_dimension)
        self.beta_layer = nn.Linear(input_dimension, output_dimension)
    ...
    def forward(self, x: torch.Tensor, conditional: torch.Tensor) -> torch.Tensor:
        ...
        assert x.shape == gamma.shape
        assert x.shape == beta.shape

        return x * (1 + gamma) + beta

class _DLinearFiLMModule(PLMixedCovariatesModule):
    """
    DLinear module
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.FiLM_seasonal = FiLM(
            self.input_dim * self.input_chunk_length, layer_out_dim
        )
    ...
    def forward(
        ...
        if self.shared_weights:
            ...
            if seasonal_output.isnan().any() or trend_output.isnan().any():
                breakpoint()
            
            seasonal_output = self.FiLM_seasonal(seasonal_output, res.view(batch, -1))
            trend_output = self.FiLM_trend(trend_output, trend.view(batch, -1))
...
class DLinearFiLMModel(MixedCovariatesTorchModel):
    def __init__(
        self,
